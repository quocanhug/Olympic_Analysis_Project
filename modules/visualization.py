...
def plot_gender_trend(df):
    ...
    plt.plot(data.index, data['F'], marker='o',
             label='Nữ', linewidth=2, color='red')
...
def plot_physical_distribution(df):
    ...
    sns.histplot(data=df, x='Height', bins=30,
                 kde=True, color='#3498db', ax=axes[1])
    axes[1].set_title('Phân bố Chiều cao (Height)')
    axes[1].set_xlabel('Chiều cao (cm)')
    axes[1].axvline(df['Height'].mean(), color='red',
                    linestyle='--', label='Trung bình')
    axes[1].legend()

    # 3. Phân bố Cân nặng
    sns.histplot(data=df, x='Weight', bins=30,
                 kde=True, color='#2ecc71', ax=axes[2])
    axes[2].set_title('Phân bố Cân nặng (Weight)')
    axes[2].set_xlabel('Cân nặng (kg)')
    axes[2].axvline(df['Weight'].mean(), color='red',
                    linestyle='--', label='Trung bình')
    axes[2].legend()

    plt.suptitle('Tổng quan Thể chất Vận động viên Olympic (Toàn bộ các môn)',
...
def plot_physical_comparison_by_sport(df):
    ...
    sns.boxplot(data=df_top, x='Sport', y='Height',
                ax=axes[0], palette='viridis', order=top_sports)
    axes[0].set_title(
        'So sánh Chiều cao giữa Top 15 môn thể thao phổ biến', fontsize=14)
    ...
    sns.boxplot(data=df_top, x='Sport', y='Weight',
                ax=axes[1], palette='magma', order=top_sports)
    axes[1].set_title(
        'So sánh Cân nặng giữa Top 15 môn thể thao phổ biến', fontsize=14)
...
def plot_host_advantage_china(df):
    ...
    medals = df_chn[df_chn['Medal'].isin(['Gold', 'Silver', 'Bronze'])].groupby(
        'Year')['Medal'].count()

    plt.figure()
    colors = ['#ff9999' if x != 2008 else '#CC0000' for x in medals.index]
    bars = plt.bar(medals.index, medals.values,
...
def plot_body_evolution_100m(df):
    ...
    subset = df[(df['Event'] == "Athletics Men's 100 metres") & (
        df['Year'] > 1900)].dropna(subset=['Height', 'Weight'])
    fig, ax1 = plt.subplots(figsize=(12, 7))

    sns.regplot(data=subset, x='Year', y='Height', ax=ax1, scatter_kws={
                'alpha': 0.3, 'color': 'blue'}, line_kws={'color': 'blue'}, label='Chiều cao')
    ax1.set_ylabel('Chiều cao (cm)', color='blue')

    ax2 = ax1.twinx()
    sns.regplot(data=subset, x='Year', y='Weight', ax=ax2, scatter_kws={
                'alpha': 0.3, 'color': 'orange'}, line_kws={'color': 'orange'}, label='Cân nặng')
...
def plot_vietnam_stats(df):
    ...
    sns.barplot(x=vn_part.index, y=vn_part.values,
                ax=ax1, color='#DA251D', alpha=0.8)
...
def plot_vietnam_details(df):
    ...
    for _, row in medals.iterrows():
        cell_text.append([row['Year'], row['Name'].split(
            '(')[0], row['Sport'], row['Medal']])
    ...
    table = ax.table(cellText=cell_text, colLabels=[
                     "Năm", "VĐV", "Môn", "Huy chương"], loc='center', cellLoc='center')
